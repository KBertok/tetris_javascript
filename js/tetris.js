...
function playerReset() {
    const piezas='ITOZSLJ';
    dropInterval = 1000 - (player.level*100);
    if (player.next === null) {
        player.matriz = createPiece(piezas[piezas.length * Math.random() | 0]);    
    } else {
        player.matriz = player.next;
    }
    player.next = createPiece(piezas[piezas.length * Math.random() | 0]);
    //player.matriz = createPiece(piezas[piezas.length * Math.random() | 0]);
...
document.addEventListener("keydown", event => {
    if (event.keyCode===40) {
        ...
        playerMove(1);    
    } else if (event.keyCode===32) { //espacio para rotar pieza
        playerRotate();
    }
